...
client = Client("https://api.example.com";
                auth = BearerToken(ENV["JAOEU_TOKEN"]))
```

`resolve_credentials` picks up env vars (`JAOEU_TOKEN`,
`JAOEU_API_KEY`, `JAOEU_USERNAME` / `JAOEU_PASSWORD`)
or falls back to `~/.config/jaoeu/credentials.toml`:
...
- [ ] Replace the placeholder `ENV["JAOEU_TOKEN"]` references
