...
@testset "TokenBucket immediate acquire" begin
    clk = _mock_clock()
    b = JAOEU.TokenBucket(; rate = 10.0, burst = 5.0)
    JAOEU.acquire!(b; sleep_fn = clk.sleep_fn, time_fn = clk.time_fn)
...
@testset "TokenBucket waits when empty" begin
    clk = _mock_clock()
    b = JAOEU.TokenBucket(; rate = 2.0, burst = 1.0)
    JAOEU.acquire!(b; sleep_fn = clk.sleep_fn, time_fn = clk.time_fn)
    JAOEU.acquire!(b; sleep_fn = clk.sleep_fn, time_fn = clk.time_fn)
...
@testset "TokenBucket throws when timeout would expire" begin
    clk = _mock_clock()
    b = JAOEU.TokenBucket(; rate = 1.0, burst = 1.0)
    JAOEU.acquire!(b; sleep_fn = clk.sleep_fn, time_fn = clk.time_fn)
    @test_throws JAOEU.RateLimitError JAOEU.acquire!(
...
@testset "with_rate_limit runs fn after acquire" begin
    clk = _mock_clock()
    b = JAOEU.TokenBucket(; rate = 100.0, burst = 5.0)
    n = Ref(0)
    JAOEU.with_rate_limit(b, () -> n[] += 1;
