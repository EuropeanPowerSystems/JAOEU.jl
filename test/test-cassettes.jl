...
function _run_cassette_tests(BrokenRecord)
    BrokenRecord.configure!(;
        ...
        #    don't end up on disk if a recording is run with a real token
        #    set (BrokenRecord still scrubs the *recorded* value).
    ...
    #         list_pets(JAOEU.Client("https://api.example.com";
    #                                  auth = JAOEU.NoAuth()))
    #     end
    #     @test !isempty(pets)
    # end
