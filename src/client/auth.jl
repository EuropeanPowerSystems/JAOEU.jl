...
"""
    resolve_credentials(T::Type{<:Auth}; env_prefix="JAOEU") -> T
...
Build an [`Auth`](@ref) of type `T` from the first available source:
  ...
  2. `~/.config/jaoeu/credentials.toml` keyed under the auth
